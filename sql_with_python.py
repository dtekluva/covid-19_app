...
def create_tables():
    ...
    with connection.cursor() as cursor:
        table_for_countries = "create table countries (id int primary key not null auto_increment, name varchar(100), longitude float, latitude float)"
        cursor.execute(table_for_countries)

        table_for_deaths = "create table deaths (id int primary key not null auto_increment, country_id int(100), foreign key(country_id) references countries(id), deaths int, date_of_death date)"
